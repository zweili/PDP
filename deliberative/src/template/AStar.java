...
// A* algorithm
	...
	public Plan AStarPlan(int capacity) {
				
		long A_start = System.currentTimeMillis();
		System.out.println("Planning with AStar... \n");
		...
		while (true) {
			...
			// get first element in Q as current state and remove it from Q
			curr_state = Q.poll();
			...
			if ((found_state == null) || (curr_state.minTotalCost < found_state.minTotalCost)) {
				...
				ArrayList<States> S = new ArrayList<States>();
				...
				ArrayList<States> new_Q = new ArrayList<States>(Q);
				new_Q.addAll(S);
				Collections.sort(new_Q);
				Q.clear();
				Q.addAll(new_Q);
				...
				for(Task task : available_tasks) {
					if(task.pickupCity == curr_city) {
						...
						if(picked_tasks.weightSum()<capacity) {
				...
				for(Task task : picked_tasks) {
					...
					if(task.deliveryCity == curr_city) {
						...
						//TaskSet removed_task = TaskSet.create(task_);
						//TaskSet new_picked_tasks = TaskSet.intersectComplement(picked_tasks, removed_task);
						//States succ_state =new States(curr_city, available_tasks, new_picked_tasks, curr_total_cost, curr_state);
						Q.add(succ_state);
		...
		System.out.print("Execution Time: " +(A_end-A_start)+ "\n");
		return bfs_plan;
	}
	...
	public void generatePlan(States curr_state, Plan plan) {
		...
		if(parent_state != null) {	// reached the first state if it is null
			...
			// delivery
			else if(parent_state.cTask.rewardSum() > curr_state.cTask.rewardSum() ) {
